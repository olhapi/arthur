...
import { nativeHostTargets } from "./install.mjs";

function missing(error) {
  return error && typeof error === "object" && error.code === "ENOENT";
}

export async function buildUninstallPlan({ home, platform, targets } = {}) {
  const expected = nativeHostTargets({ home, platform });
  ...
  return { targets: allowlist, nativeHostDirectory: path.dirname(expected.binary) };
}

export async function applyUninstallPlan(plan, { fs = nodeFs } = {}) {
  ...
  for (const target of plan.targets) {
    try { await fs.unlink(target); } catch (error) { if (!missing(error)) throw error; }
  }
  try {
    if ((await fs.readdir(plan.nativeHostDirectory)).length === 0) await fs.rmdir(plan.nativeHostDirectory);
  } catch (error) {
    if (!missing(error)) throw error;
  }
}
...
async function main({ argv, env, platform, fs }) {
  ...
  const plan = await buildUninstallPlan({ home: env.HOME, platform });
  await applyUninstallPlan(plan, { fs });
}
