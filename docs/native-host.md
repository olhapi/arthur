...
Installation is recoverable: the binary and each manifest are staged in their final directory, fsynced, atomically renamed, then their parent directory is fsynced before the next target. A parent-directory sync failure is reported as an incomplete install and is never hidden as success. Uninstall is bounded. It can remove only these exact four Arthur-owned files: the single binary and the three manifest files above. It leaves parent browser and application-support directories in place.
