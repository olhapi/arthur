...
Before installation, each npm package must be checked with `pnpm view <package> version time deprecated`, and each Rust crate with `cargo search` plus `cargo info`; every primary repository must be checked for recent activity and advisories. The resolved project must pass both `pnpm audit` and `cargo audit`. A dependency-free build-time adapter resolves Homebrew's keg-only rustup with `brew --prefix rustup`, asks that rustup for the exact Cargo and rustc binaries in toolchain 1.97.1, verifies both versions, and spawns Cargo with the resolved toolchain directory prepended to a copied `PATH` plus `RUSTC` set to the resolved compiler. Native commands never rely on whichever Cargo/rustc appears first in the parent environment. `cargo-audit` 0.22.2 is installed in the standard Cargo bin, but every implementation/release run must verify that exact version and reinstall it through the adapter only if it is absent or mismatched. Inability to resolve rustup, select Rust 1.97.1, verify cargo-audit 0.22.2, or run it against `native/Cargo.lock` blocks the task and must never become a skipped or weakened gate. This adapter is build-time only and does not add Homebrew, rustup, Cargo, Node, or a shell environment requirement to the installed host. Any dependency beyond the two lists requires a documented feature-specific reason.
