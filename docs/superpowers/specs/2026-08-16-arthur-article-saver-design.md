...
Before installation, each npm package must be checked with `pnpm view <package> version time deprecated`, and each Rust crate with `cargo search` plus `cargo info`; every primary repository must be checked for recent activity and advisories. The resolved project must pass both `pnpm audit` and `cargo audit`. `cargo-audit` 0.22.2 is not currently installed, so implementation must explicitly install that exact locked tool, verify its version, and run it against `native/Cargo.lock`; inability to install or run it blocks the task and must never be converted into a skipped or weakened gate. Any dependency beyond the two lists requires a documented feature-specific reason.
